...
const ul = document.querySelector('#tab3>ul'); // 게시물을 담을 unordered list
...
const setPageOf = (pageNumber) => {
  ...
  for (
    ...
    // 글 날짜
    const postDay = document.createElement('p');
    postDay.className = 'post-day';

    // 글 조회수
    const postCheck = document.createElement('p');
    postCheck.className = 'post-check';

    postNumber.textContent = data[i - 1].postNumber;
    postTitle.textContent = data[i - 1].title;
    postDay.textContent = data[i - 1].postDay;
    postCheck.textContent = data[i - 1].postCheck;

    postContainer.append(postNumber, postTitle, postDay, postCheck);
